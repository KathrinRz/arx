...
public class ConfigRandomShiftDecimalMasker{
	 ...
	 /**
		 * Methode, die ein Coposite erzeugt, in dem ein Textfeld und eine neue Composite für eine Verteilung erstellt werden
		 * @param s1
		 * @param x
		 * @param y
		 */
		
	 public ConfigRandomShiftDecimalMasker(Shell s1, int x, int y){
			...
			btn1.addSelectionListener(new SelectionAdapter(){
				public void widgetSelected(SelectionEvent e){
					boolean istOK = cont1.getOK() && eingabeOK;
					if(istOK==true)	{
						btn1.setText("OK");
					}
					else{
		...
		cont1 = new ContinousDistributionSelection(root,0);
		cont1.addModifyListener(new ModifyListener(){
		   	public void modifyText(ModifyEvent e){
		   		 btn1.setEnabled(cont1.getOK());
		    	}
		    });	
		}
